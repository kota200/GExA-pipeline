...
out["treatment"] = out["treatment"].replace({"":"NA", "no_treatment":"control"})
out["cultivar"] = out["cultivar"].replace({"":"NA", "Cliper":"Clipper", "cultivar_Rolap":"Rolap", "variety_Igri":"Igri"})
out["temperature"]= out["temperature"].replace({"":"NA"})
out["tissue"] = out["tissue"].replace({"na":"NA", "leaves":"leaf","spikes":"spike"})
out["stage"] = out["stage"].replace({"na":"NA", "Zadok_11_(first_leaf_emerged)":"first_leaf_emerged"})
for c in cols_clean:
